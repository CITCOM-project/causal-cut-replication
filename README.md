...
1. To run on HPC with SLURM, run `bash hpc-submissions.sh`. To run locally, run `bash local_run.sh`. N.B. This will take a few days, maybe up to a week. To collect a subset of the data, change the range of `for i in {0..30}` to something smaller, e.g. `for i in {0..5}`, or simply kill the process when you run out of time.
